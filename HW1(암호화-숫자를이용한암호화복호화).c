...
void DecodedSentence(char sentence[], char alphabet[], char encoded[], int s_len, int distance)
{
	int i, j = 0;
	...
	while(j < s_len){
		if (sentence[j] == ' ')
			...
			for (i = 0; i < 26; i++)
				if (sentence[j] == encoded[i]){
					if (i+distance < 26)
						printf("%c", alphabet[i+distance]);
					else
						printf("%c", alphabet[i+distance-26]);
				}
		}
		++j;
	}
...
int main(void)
{
	...
	DecodedSentence(sentence, alphabet, encoded, sizeof(sentence), distance);
}